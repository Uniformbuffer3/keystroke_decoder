...
impl KeystrokeDecoder {
    ...
    pub fn decode(&mut self, keycode: u32) -> Keystrokes {
        let direction = match self
            .keystroke_stack
            .iter()
            .rposition(|element| element == &keycode)
        {
            Some(index) => {
    ...
    pub fn is_caps_lock_pressed(&self) -> bool {
        self.state
            .mod_name_is_active(&xkb::MOD_NAME_CTRL, xkb::STATE_MODS_EFFECTIVE)
    }
    pub fn is_num_lock_pressed(&self) -> bool {
        self.state
            .mod_name_is_active(&xkb::MOD_NAME_CTRL, xkb::STATE_MODS_EFFECTIVE)
    }
}
            ...
            KeyDirection::Down => {
                ...
                if self.is_shift_pressed() {
                    chars
                        .into_iter()
                        .map(|character| {
                            character.to_uppercase().into_iter().collect::<Vec<char>>()
                        })
                        .flatten()
                        .collect()
                } else {
